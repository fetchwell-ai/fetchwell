...
```bash
pnpm test:unit                        # Vitest — 215 unit tests, <1s
...
- **Stanford E2E tests:** Both require Stanford creds in `providers.json`, bundled API key (or `ANTHROPIC_API_KEY` for CLI), and `E2E_STANFORD=1`. Skipped otherwise. Both steal focus.
  - `tests/e2e/stanford-e2e.spec.ts` — add Stanford portal → extract. ~2-4 min.
  - `tests/e2e/stanford-lifecycle.spec.ts` — clean slate → add → extract → delete. Uses custom fixture with **temp download folder** and `incrementalExtraction: false` so extraction is fully exercised (not skipped by pre-existing PDFs). ~5-10 min.
...
- **Electron ↔ Pipeline bridge.** Electron forks `src/electron-runner.ts` as a subprocess — via tsx in dev, or the pre-compiled `dist-electron/electron-runner.cjs` bundle when packaged. IPC for progress events, 2FA relay, and error reporting. One operation per portal at a time.
- **Packaging: `asar: false` + afterPack.** electron-builder's pnpm dependency walker misses transitive deps. `asar: false` lets `fork()` access the runner on the real filesystem. `scripts/after-pack.cjs` runs `npm install --omit=dev` inside the packaged app to get a complete flat `node_modules`. The CJS runner format + `--define:import.meta.dirname=__dirname` handles ESM→CJS API differences at build time.
- **Bundled Chromium.** Playwright Chromium is bundled via `extraResources` (~200MB). System Chrome (`channel: 'chrome'`) was rejected — `launchPersistentContext` crashes on macOS and conflicts with the user's running Chrome. Stagehand requires `downloadsPath` in `localBrowserLaunchOptions` because `process.cwd()` is `/` when launched from Finder.
