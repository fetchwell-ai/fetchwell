/**
 * MyChart authentication module.
 *
 * Implements the AuthModule interface for Epic MyChart portals.
 * Contains all the login, 2FA, and session-verification logic that was
 * previously in src/auth.ts.
 */
...
export async function doLogin(
  ...
  if (twoFaObservations.length > 0) {
    ...
    if (!enteredCode) {
      // File-based 2FA: write output/<provider>/2fa.needed, watch for 2fa.code.
...
/**
 * MyChart auth module implementing the pluggable AuthModule interface.
 *
 * Delegates to the existing doLogin / ensureLoggedIn functions so
 * behavior is identical to the original monolithic auth.ts.
 */
export const myChartAuth: AuthModule = {
